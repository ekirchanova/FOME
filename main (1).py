...
class MyWin(QtWidgets.QMainWindow):
    ...
    def save_graph(self):
        ...
        if (self.index == 1):
            np.savetxt('test_1.txt', self.Na, self.Electron_concentration)
        if (self.index == 2):
            np.savetxt('test_1.txt', self.Na, self.Hole_concentration)
        if (self.index == 3):
            np.savetxt('test_1.txt', self.Na, self.Negative_acceptor_concentration)
        if (self.index == 4):
            np.savetxt('test_1.txt', self.Na, self.Positive_acceptor_concentration)
        if (self.index == 5):
            np.savetxt('test_1.txt', self.Na, self.Conductivity)
        if (self.index == 6):
            np.savetxt('test_1.txt', self.Na, self.Resistivity)
